...
118. `[A]` `wandersort reset --db --yes` → `.wandersort.db.zst` written (smaller than the database), then all scan data wiped.
118a. `[A]` `wandersort recover --yes` after 118 → the wiped data is back; `.wandersort.db.zst` still there.
118b. `[A]` `wandersort recover` with no `.wandersort.db.zst` → error naming execute and `reset --db` as what writes one.
