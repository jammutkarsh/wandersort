...
func (m pickerModel) handleKey(key tea.KeyMsg) (tea.Model, tea.Cmd) {
	m.quitWarned = m.quitWarned && (key.String() == "q" || key.String() == "ctrl+c")
	switch key.String() {
	case "q", "ctrl+c":
		// ctrl+c is the app's quit key everywhere, so it goes straight out;
		// [q] warns once while slices are still unreviewed.
		if left := m.unsaved(); left > 0 && key.String() == "q" && !m.quitWarned {
			m.quitWarned = true
			m.status = fmt.Sprintf("%d time slices not saved yet — press q again to leave them", left)
			m.statusErr = true
			return m, nil
		}
...
// reopen puts a saved segment back to reviewable — the only way back into a
// slice that was signed off, short of rebuilding the whole proposal.
...
func (m pickerModel) View() string {
	...
	if m.unsaved() == 0 {
		left = "Every time slice is saved. [R] re-opens one, [q] leaves."
	}
	...
	foot = append(foot, tui.Footer(strings.Join([]string{
		tui.KeyHint("↑↓", "move"),
		tui.KeyHint("enter", "review this slice"),
		tui.KeyHint("R", "re-open a saved slice"),
		tui.KeyHint("q", "leave"),
		tui.KeyHint("ctrl+c", "quit"),
	}, "   "), m.w))
